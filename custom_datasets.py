...
class ToxicComment(Dataset):
    """
    Customized dataset class for Toxic Comment Classification.
    ...
    def __init__(self, tokenizer, seq_len=SEQ_LEN, split="train", n_rows: int = None):
        """
        Initializes the ToxicComment dataset class.
        ...
        if self.n_rows is not None:
            ...
            self.dataset = load_dataset(
                "jigsaw_toxicity_pred", data_dir=TOXIC, split=f"{self.split}{n_rows_str}")
        else:
            self.dataset = load_dataset(
                "jigsaw_toxicity_pred", data_dir=TOXIC)
    ...
    def __getitem__(self, item):
        """
        Obtains comments from the dataset, applies a tokenization and appends labels

        Args:
            item (int): index of the datum

        Returns:
            dict: Processed input with tokenization and label
        """
